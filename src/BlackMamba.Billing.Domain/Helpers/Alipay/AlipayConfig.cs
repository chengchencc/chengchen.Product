...
namespace Alipay.Class
{
    ...
    public class Config
    {
        static Config()
        {
            ...
            #region 商户需要手动配置
            Partner = ConfigKeys.ALI_PARTNER_NO.ConfigValue();
            Seller_account_name = ConfigKeys.ALI_SELLER_ACCOUNT.ConfigValue();
            PrivateKey = ConfigKeys.MERCHANT_PRIVATE_KEY.ConfigValue();
            Alipaypublick = ConfigKeys.ALI_PUBLIC_KEY.ConfigValue();
            //三个返回URL
            Call_back_url = ConfigKeys.ALI_WAP_PAY_CALLBACK_URL.ConfigValue();
            Notify_url = ConfigKeys.ALI_WAP_PAY_NOTIFY_URL.ConfigValue();
            Merchant_url = ConfigKeys.ALI_WAP_PAY_MERCHANT_URL.ConfigValue();
