...
namespace Alipay.Class
{
    ...
    public class Function
    {
        ...
        public static string BuildMysign(SortedDictionary<string, string> dicArrayPre, string privatekey, string input_charset)
        {
            ...
            mysign = System.Web.HttpUtility.UrlEncode(mysign, Encoding.GetEncoding(input_charset)); //此处需要对签名进行Encode，否则出现+号等特殊字符，通过base64转换并post提交给支付宝服务器会丢失，变成空格
            return mysign;
        }
        ...
        public static string GetStrForXmlDoc(XmlDocument xmlDoc, string xmlNode)
        {
            XmlNode xn = xmlDoc.SelectSingleNode(xmlNode);
            return xn == null ? string.Empty : xn.InnerText.MakeSureNotNull();
        }
