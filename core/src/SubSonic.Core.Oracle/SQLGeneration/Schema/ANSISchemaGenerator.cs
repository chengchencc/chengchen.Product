...
namespace SubSonic.Oracle.SqlGeneration.Schema
{
    ...
    public abstract class ANSISchemaGenerator : ISchemaGenerator
    {
        ...
        public abstract string GenerateInsertProcedure(ITable table);


        public virtual string BuildAddIndexStatement(IColumn column)
        {
            throw new NotImplementedException();
        }

        public virtual string BuildAlterIndexStatement(IColumn column)
        {
            throw new NotImplementedException();
        }

        public virtual string BuildDropIndexStatement(IColumn column)
        {
            throw new NotImplementedException();
        }

        public string GetDefaultIndexName(IColumn column)
        {
            // make sure index name is less or equal than 30 characters
            var tableColName = column.Table.Name.ToUpper() + "_" + column.Name.ToUpper();
            if (tableColName.Length > 27) tableColName = tableColName.Substring(0, 27);

            return string.IsNullOrEmpty(column.IndexName) ? tableColName + "_IX" : column.IndexName;
        }
    }
}