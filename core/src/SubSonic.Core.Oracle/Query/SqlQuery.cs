...
namespace SubSonic.Oracle.Query
{
    ...
    public class SqlQuery : ISqlQuery
    {
        ...
        public SqlQuery Where<T>(Expression<Func<T, bool>> expression)
        {
            ...
            var hash = Objects.ToColumnDictionary<T>();
            foreach(Constraint constrain in c)
            {
                string columnName = constrain.ColumnName;
                if (string.IsNullOrEmpty(columnName))
                    columnName = constrain.ConstructionFragment;
                columnName = hash[columnName];
                IColumn column = _provider.FindTable(typeof(T).Name).GetColumnByPropertyName(columnName);
        ...
        internal static void SetConstraintParams(SqlQuery qry, QueryCommand cmd)
        {
            ...
            foreach(Constraint c in qry.Constraints)
            {
                ...
                IColumn col = qry.FindColumn(c.ConstructionFragment);
                if(col != null)
