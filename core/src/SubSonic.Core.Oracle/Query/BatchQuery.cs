...
namespace SubSonic.Oracle.Query
{
    /// <summary>
                    ...
                    foreach (var cmd in _fixedCommands)
                    {
                        if (!String.IsNullOrEmpty(cmd.CommandSql))
                        {
                            ...
                            try
                            {
                                ...
                                if (!dbCommand.CommandText.StartsWith("DECLARE BEGIN EXECUTE IMMEDIATE '"))
                                {
                                    dbCommand.CommandText = dbCommand.CommandText.Replace("''", "'");
                                }
                            ...
                            catch (Exception ex)
                            {
                                ...
                                //ORA-22859 无效的列修改,修改字段类型必需是同一种类型，只能扩大不能缩小。不同类型的字段修改必需保证被修改的字段为空（没有数据），不是主键。当能要先备份数据再修改，否则可能会丢失数据。修改字段类型必需是同一种类型，只能扩大不能缩小。不同类型的字段修改必需保证被修改的字段为空（没有数据），不是主键。当能要先备份数据再修改，否则可能会丢失数据。
                                //ORA-01440: 要减小精度或标度, 则要修改的列必须为空
                                //ORA-01408: 此列列表已索引
                                if (error.Contains("ORA-22859") || error.Contains("ORA-01440") || error.Contains("ORA-01408"))
                                {
                                    // we need log this error infomation.
