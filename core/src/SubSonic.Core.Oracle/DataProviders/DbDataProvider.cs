...
namespace SubSonic.Oracle.DataProviders
{
    public abstract class DbDataProvider : IDataProvider
    {
        ...
        protected DbDataProvider(string connectionString, string providerName)
        {
            ...
            // subsonic's to do: Schema is specific to SQL Server?
        ...
        public DbProviderFactory Factory
        {
            get
            {
                return DbProviderFactories.GetFactory(DbDataProviderName);
            }
        }
        ...
        {
            // subsonic's to do:: Remove this check
        ...
        private static void AddParams(DbCommand cmd, QueryCommand qry)
        {
            if (qry.Parameters != null)
            {
                foreach (QueryParameter param in qry.Parameters)
                {
                    ...
                    if (param.DataType == DbType.Boolean)
                    {
                    ...
                    else if (param.DataType == DbType.Boolean)
                    {
                        if (param.ParameterValue.GetType().Equals(typeof(DBNull)))
                        {
                            p.Value = DBNull.Value;
                        }
                        else
                        {
                            p.Value = (bool)param.ParameterValue ? 1 : 0;
                        }
                    }
                    else
        ...
        public virtual IEnumerable<T> ToEnumerable<T>(QueryCommand<T> query, object[] paramValues)
        {
            ...
            // subsonic's to do: Can we use Database.ToEnumerable here? -> See commit 654aa2f48a67ba537e34 that fixes some issues
            Type type = typeof(T);
