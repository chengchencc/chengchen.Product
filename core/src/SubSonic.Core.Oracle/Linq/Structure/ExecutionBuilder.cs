...
namespace SubSonic.Oracle.Linq.Structure
{
    /// <summary>
    ...
    public class ExecutionBuilder : DbExpressionVisitor
    {
        ...
        protected override Expression VisitColumn(ColumnExpression column)
        {
            ...
            if (scope.TryGetValue(column, out reader, out iOrdinal))
            {
                ...
                {
                    var getValueMethod = Expression.Call(reader, "GetValue", null, Expression.Constant(iOrdinal));
                    var getValueToString = Expression.Call(typeof(Convert), "ToString", null, getValueMethod);
                    
                    value = Expression.Convert(
                                Expression.Call(typeof(Enum), "Parse", null,
