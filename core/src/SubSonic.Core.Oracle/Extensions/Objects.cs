...
namespace SubSonic.Oracle.Extensions
{
    public static class Objects
    {
        ...
        public static Dictionary<string, object> ToDictionary(this object value)
        {
            ...
            var fields = value.ToColumnDictionary();
            foreach(PropertyInfo pi in props)
            {
                try
                {
                    ...
                    if (!HasIgnoreAttr(pi))
                    {
                        result.Add(fields[pi.Name], pi.GetValue(value, null));
                    }
                    #endregion
        ...
        private static bool HasIgnoreAttr(PropertyInfo pi)
        {
            var columnAttributes = pi.GetCustomAttributes(typeof(SubSonicIgnoreAttribute), false);
        ...
        public static T FromDictionary<T>(this Dictionary<string, object> settings, T item) where T : class
        {
            ...
            var fields = item.ToColumnDictionary();
            //FieldInfo[] fields = item.GetType().GetFields();
            foreach(PropertyInfo pi in props)
            {
                #region Default

                if (settings.ContainsKey(fields[pi.Name]))
                {
                    if (pi.CanWrite)
                        pi.SetValue(item, settings[fields[pi.Name]], null);
                }
                #endregion
        ...
        public static Dictionary<string, string> ToColumnDictionary(this object value)
        {
            ...
            foreach (PropertyInfo pi in props)
            {
                try
                {
                    ...
                    if (columnAttributes.Length == 0)
                        result.Add(pi.Name, pi.Name);
                    else
                    {
                        result.Add(pi.Name, ((SubSonicColumnNameOverrideAttribute[])columnAttributes)[0].ColumnName);
                    }
        ...
        public static Dictionary<string, string> ToColumnDictionary<T>()
        {
            Dictionary<string, string> result = new Dictionary<string, string>();
    	...
    	public static ITable ToSchemaTable(this Type type, IDataProvider provider)
        {
            ...
            var partitionAttributes = type.GetCustomAttributes(typeof(IPartitionMappingAttribute), false);
            foreach (IPartitionMappingAttribute attr in partitionAttributes)
            {
                if (attr.Accept(result))
                {
                    attr.Apply(result);
        ...
        private static bool ColumnIsIgnored(object[] attributes)
        {
            ...
            {
                if (att.ToString().Equals(typeof(SubSonicIgnoreAttribute).FullName))
                {
                    return true;
                }
            }
