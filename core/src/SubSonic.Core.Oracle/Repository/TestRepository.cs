...
namespace SubSonic.Oracle.Repository
{
    public class TestRepository<T> : IRepository<T> where T : IActiveRecord, new()
    {
        ...
        public object Add(T item, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return Add(item);
        }

        public void Add(IEnumerable<T> items, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            Add(items);
        }

        public int Update(T item, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return Update(item);
        }

        public int Update(IEnumerable<T> items, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return Update(items);
        }

        public int Delete(T item, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return Delete(item);
        }

        public int Delete(object key, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return Delete(key);
        }

        public int DeleteMany(Expression<Func<T, bool>> expression, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return DeleteMany(expression);
        }

        public int Delete(IEnumerable<T> items, SubSonic.Oracle.DataProviders.IDataProvider provider) {
            return Delete(items);
        }
